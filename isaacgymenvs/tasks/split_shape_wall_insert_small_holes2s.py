...
class SplitShapeWallInsertSmallHoles2S(VecTask):
    ...
    def init_data(self):
        ...
        self._obj_hex_length_torch = torch.ones_like(self._eef_state[:, 0]) * self.obj_hex_length
        self._obj_hex_diam_torch = torch.ones_like(self._eef_state[:, 0]) * self.obj_hex_diam
        self._obj_hex_goal_grip_pos_x = torch.ones_like(self._eef_state[:, :3])
        self._obj_hex_goal_grip_pos_x[:, 0] = -self._obj_hex_length_torch/2
        # Initialize states
        self.states.update({
    ...
    def _update_states(self):
        self.states.update({
            ...
            "cube_goal_grip_pos_xyz": self._cube_state[:, :3] + self._cube_goal_grip_pos_x[:, :3],
            ...
            "obj_hex_pos_xyz": self._obj_hex_state[:, :3],
            "obj_hex_goal_grip_pos_xyz": self._obj_hex_state[:, :3] + self._obj_hex_goal_grip_pos_x[:, :3],
            "obj_hex_goal_grip_pos_to_gripper_xyz": self._obj_hex_state[:, :3] + self._obj_hex_goal_grip_pos_x - - self._eef_state[:, :3],
            "obj_hex_to_gripper_xyz": self._obj_hex_state[:, :3] - self._eef_state[:, :3], # obj_hex position relative to hand. How far is obj_hex A from hand.
            ...
            "obj_hex_to_hex_bottom_xyz": self._obj_hex_state[:, :3] - self._hex_bottom_state[:, :3],
    ...
    def compute_observations(self):
        ...
        obs = ["cube_quat", "cube_pos_xyz", "cube_to_square_bottom_xyz", "obj_hex_to_hex_bottom_xyz", "eef_pos", "eef_quat"]
...
def compute_franka_reward(
    ...
    # cube_grip_goal_pose = torch.zeros_like(states["cube_pos_xyz"])
    # cube_grip_goal_pose_x[: 0] = -cube_length/2
    # cube_grip_edge_goal = states["cube_pos_xyz"] + cube_grip_goal_pose_x
    ...
    max_reward_bw_gripper_cube_align = torch.max(cube_gripping_reward, cube_align_reward)

    # cube XY alignment 
    cube_y_alignment  = torch.abs(states["cube_to_square_bottom_xyz"][:, 0]) < .05
    cube_x_alignment = torch.abs(states["cube_to_square_bottom_xyz"][:, 1]) < .075
    ...
    cube_placement_complete = cube_x_alignment & cube_y_alignment & dist_cube_to_square_hole_z & gripper_released_cube

    # Compose rewards
    scaled_cube_placement_complete = reward_settings["r_stack_scale"] * cube_placement_complete
    scaled_cube_dist_reward = reward_settings["r_dist_scale"] * max_reward_bw_gripper_cube_align
    scaled_cube_lifed_reward = reward_settings["r_lift_scale"] * is_cube_lifted
    scaled_cube_alignment_reward = reward_settings["r_align_scale"] * cube_align_reward
    scaled_cube_mid_reward = scaled_cube_dist_reward + scaled_cube_lifed_reward + scaled_cube_alignment_reward
    # if arg0 is true, arg1, else arg2
    cube_rewards = torch.where(
        cube_placement_complete, 
        scaled_cube_placement_complete,
        scaled_cube_mid_reward
    )
    ...
    if progress_buf[0].item() == (max_episode_length-1):
        for i in range(1):
            ...
            print_str += "{}, ".format(cube_align_reward[i].item())
            print_str += "{}, ".format(max_reward_bw_gripper_cube_align[i].item())
            ...
            print_str += "{}, ".format(cube_rewards[i].item())
            print(print_str)
            ...
            print(f"Alignment Metrics || Alignment Reward: {cube_align_reward[i].item()}")
            print(f"      cube_pos_xyz:                 [{states['cube_pos_xyz'][i, 0].item()}, {states['cube_pos_xyz'][i, 1].item()}, {states['cube_pos_xyz'][i, 2].item()}]")
            ...
            print("       cube_x_alignment:                ", cube_x_alignment[i].item())
            print("       cube_y_alignment:                ", cube_y_alignment[i].item())
            ...
            print("Reward Pieces")
            print(f"      dist_reward:                  {scaled_cube_dist_reward[0].item()}")
            print(f"      lift_reward:                  {scaled_cube_dist_reward[0].item()}")
            print(f"      alignment_reward:             {scaled_cube_alignment_reward[0].item()}")


    ################# HEXAGON REWARD SECTION ######################################


    hex_height = states["obj_hex_diam"]    
    ...
    # Reward for gripping the hex
    # hex_grip_goal_pose = torch.zeros_like(states["hex_pos_xyz"])
    # hex_grip_goal_pose_x[: 0] = -hex_length/2
    # hex_grip_edge_goal = states["hex_pos_xyz"] + hex_grip_goal_pose_x
    # dist_hex_to_gripper = torch.norm(states["hex_to_gripper_xyz"], dim=-1)
    # dist_hex_to_left_finger = torch.norm(states["hex_pos_xyz"] - states["eef_lf_pos_xyz"], dim=-1)
    # dist_hex_to_right_finger = torch.norm(states["hex_pos_xyz"] - states["eef_rf_pos_xyz"], dim=-1)
    # hex_gripping_reward = 1 - torch.tanh(10.0 * (dist_hex_to_gripper+ dist_hex_to_left_finger + dist_hex_to_right_finger) / 3)

    dist_hex_to_gripper = torch.norm(states["hex_goal_grip_pos_to_gripper_xyz"], dim=-1)
    dist_hex_to_left_finger = torch.norm(states["hex_goal_grip_pos_xyz"] - states["eef_lf_pos_xyz"], dim=-1)
    dist_hex_to_right_finger = torch.norm(states["hex_goal_grip_pos_xyz"] - states["eef_rf_pos_xyz"], dim=-1)
    hex_gripping_reward = (1 - torch.tanh(10.0 * (dist_hex_to_gripper+ dist_hex_to_left_finger + dist_hex_to_right_finger) / 3))*scaled_cube_placement_complete

    # Reward for lifting hex
    hex_lifting_moe = 0.04
    hex_height_from_table = states["hex_pos_xyz"][:, 2] - reward_settings["table_height"]
    hex_height_from_ground = states["hex_pos_xyz"][:, 2]
    hex_lifted = (hex_height_from_table - hex_height) > hex_lifting_moe
    is_hex_lifted = hex_lifted * scaled_cube_placement_complete

    # how closely aligned hex is to cubby (only provided if hex is lifted) num_envs X 3 
    hex_goal_position_z = torch.zeros_like(states["hex_hole_bottom_xyz"])
    hex_goal_position_z[:, 2] = hex_height/2
    hex_goal_position_xyz = states["hex_hole_bottom_xyz"] + hex_goal_position_z
    dist_hex_to_goal = torch.norm(states["hex_pos_xyz"] - hex_goal_position_xyz, dim=-1)
    hex_align_reward = (1 - torch.tanh(10.0 * dist_hex_to_goal)) * is_hex_lifted * scaled_cube_placement_complete
    
    # Dist reward is maximum of dist and align reward
    max_reward_bw_gripper_hex_align = torch.max(hex_gripping_reward, hex_align_reward)

    # hex XY alignment 
    hex_y_alignment  = torch.abs(states["hex_to_hex_bottom_xyz"][:, 0]) < .05
    hex_x_alignment = torch.abs(states["hex_to_hex_bottom_xyz"][:, 1]) < .075

    xy_hex_to_hex_hole = states["hex_to_hex_bottom_xyz"][:, :2]
    dist_hex_to_hex_hole_xy = (torch.norm(xy_hex_to_hex_hole, dim=-1) < 0.02)

    # hex Z alignment 
    dist_hex_to_hex_hole_z = torch.abs(hex_height_from_ground - target_hex_height) < 0.02
    
    # gripper releases hex 
    gripper_released_hex = (dist_hex_to_gripper > 0.04)

    # stack reward is a boolean condition that tells us if we have succeeded
    # hex_placement_complete = dist_hex_to_hex_hole_xy & dist_hex_to_hex_hole_z & gripper_released_hex
    hex_placement_complete = hex_x_alignment & hex_y_alignment & dist_hex_to_hex_hole_z & gripper_released_hex & scaled_cube_placement_complete

    # Compose rewards
    scaled_hex_placement_complete = reward_settings["r_stack_scale"] * hex_placement_complete
    scaled_hex_dist_reward = reward_settings["r_dist_scale"] * max_reward_bw_gripper_hex_align
    scaled_hex_lifted_reward = reward_settings["r_lift_scale"] * is_hex_lifted
    scaled_hex_alignment_reward = reward_settings["r_align_scale"] * hex_align_reward
    scaled_hex_mid_reward = scaled_hex_dist_reward + scaled_hex_lifted_reward + scaled_hex_alignment_reward
    # if arg0 is true, arg1, else arg2
    hex_rewards = torch.where(
        hex_placement_complete, 
        scaled_hex_placement_complete,
        scaled_hex_mid_reward
    )

    if progress_buf[0].item() == (max_episode_length-1):
        for i in range(1):
            if scaled_cube_placement_complete[i].item():
                print("Hit end of episode for ENV: ", i)

                print_str = ""
                print_str += "id: {}, ".format(i)
                print_str += "{}, ".format(progress_buf[i].item())
                print_str += "{}, ".format(hex_gripping_reward[i].item())
                print_str += "{}, ".format(hex_lifted[i].item())

                print_str += "{}, ".format(hex_align_reward[i].item())
                print_str += "{}, ".format(max_reward_bw_gripper_hex_align[i].item())
                print_str += "{}, ".format(dist_hex_to_hex_hole_xy[i].item())
                print_str += "{}, ".format(dist_hex_to_hex_hole_z[i].item())
                print_str += "{}, ".format(gripper_released_hex[i].item())
                print_str += "{}, ".format(hex_placement_complete[i].item())
                print_str += "{}, ".format(hex_rewards[i].item())
                print(print_str)

                print(f"Lifting Metrics || Is hex lifted:  {hex_lifted[i].item()}")
                print(f"      hex_pos_xyz:                 [{states['hex_pos_xyz'][i, 0].item()}, {states['hex_pos_xyz'][i, 1].item()}, {states['hex_pos_xyz'][i, 2].item()}]")
                print(f"      leff_finger_pos_xyz:           [{states['eef_lf_pos_xyz'][i, 0].item()}, {states['eef_lf_pos_xyz'][i, 1].item()}, {states['eef_lf_pos_xyz'][i, 2].item()}]")
                print(f"      right_finger_pos_xyz:         [{states['eef_rf_pos_xyz'][i, 0].item()}, {states['eef_rf_pos_xyz'][i, 1].item()}, {states['eef_rf_pos_xyz'][i, 2].item()}]")
                print(f"      gripper_pos_xyz:              [{states['eef_state'][i, 0].item()}, {states['eef_state'][i, 1].item()}, {states['eef_state'][i, 2].item()}]")
                ...
                print(f"Alignment Metrics || Alignment Reward: {hex_align_reward[i].item()}")
                print(f"      hex_pos_xyz:                 [{states['hex_pos_xyz'][i, 0].item()}, {states['hex_pos_xyz'][i, 1].item()}, {states['hex_pos_xyz'][i, 2].item()}]")
                print(f"      hex_goal_position_xyz:       [{hex_goal_position_xyz[i, 0].item()}, {hex_goal_position_xyz[i, 1].item()}, {hex_goal_position_xyz[i, 2].item()}]")
                print(f"      hex_hole_bottom_xyz:       [{states['hex_hole_bottom_xyz'][i, 0].item()}, {states['hex_hole_bottom_xyz'][i, 1].item()}, {states['hex_hole_bottom_xyz'][i, 2].item()}]")
                print(f"      hex_to_hex_bottom_xyz:    [{states['hex_to_hex_bottom_xyz'][i, 0].item()}, {states['hex_to_hex_bottom_xyz'][i, 1].item()}, {states['hex_to_hex_bottom_xyz'][i, 2].item()}]")
                print(f"      dist_hex_to_hex_hole_xy:  {dist_hex_to_hex_hole_xy[i].item()}")
                print(f"      dist_hex_to_hex_hole_z:   {dist_hex_to_hex_hole_z[i].item()} (hex_height_from_ground: {hex_height_from_ground[i].item()} vs target_hex_height: {target_hex_height[i].item()})")
                print("       hex_x_alignment:                ", hex_x_alignment[i].item())
                print("       hex_y_alignment:                ", hex_y_alignment[i].item())

                print(f"Release Metric || Has Gripper Released the hex?  {gripper_released_hex[i].item()}")
                print("       Did gripper yeet hex?")
                print(f"      hex_linear_velocity:         [{states['hex_vel'][i, 0].item()}, {states['hex_vel'][i, 1].item()}, {states['hex_vel'][i, 2].item()}]")
                print(f"      hex_angular_velocity:        [{states['hex_ang_vel'][i, 0].item()}, {states['hex_ang_vel'][i, 1].item()}, {states['hex_ang_vel'][i, 2].item()}]")
                
                print("hex Placement Complete?: ", hex_placement_complete[i].item())
                print("Reward Pieces")
                print(f"      dist_reward:                  {scaled_hex_dist_reward[0].item()}")
                print(f"      lift_reward:                  {scaled_hex_dist_reward[0].item()}")
                print(f"      alignment_reward:             {scaled_hex_alignment_reward[0].item()}")

    reward = hex_rewards + cube_rewards
    ...
    reset_buf = torch.where((progress_buf >= max_episode_length - 1), torch.ones_like(reset_buf), reset_buf)
            ...
            # print("r_align_scale*cube_align_reward: ", reward_settings["r_align_scale"] * cube_align_reward[i].item() )
