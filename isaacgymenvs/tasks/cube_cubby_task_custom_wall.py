...
class CubeCubbyTaskCustomWall(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # wall_asset_file = "urdf/separated_wall_shapes/square_bottom.urdf"
        wall_top_asset_file = "urdf/separated_wall_shapes/wall_top.urdf"
        # wall_asset_file = "urdf/objects/cube_multicolor_allegro_sdf.urdf"
        # wall_asset_file = "urdf/tray/tray.urdf"
        ...
        # # wall_opts.collision_margin = 0.01
        ...
        # wall as cubby bottom
        bar_options = gymapi.AssetOptions()
        bar_options.fix_base_link = True
        wall_asset = self.gym.create_box(self.sim, self.frame_bar_length, self.frame_bar_thickness, self.frame_bar_thickness, bar_options)

        # wall top
        # wall_top_opts = gymapi.AssetOptions()
        # wall_top_opts.fix_base_link = True
        # wall_top_opts.use_mesh_materials = True  
        # wall_top_asset = self.gym.load_asset(self.sim, asset_root, wall_top_asset_file, wall_top_opts)
        ...
        num_wall_bodies = self.gym.get_asset_rigid_body_count(wall_asset)
        num_wall_shapes = self.gym.get_asset_rigid_shape_count(wall_asset)
        ...
        self.wall_center_to_square_top = .1
   ...
   # wall stuff 
        ...
        self.ground_to_square_bottom = self.table_height + table_thickness/2 + self.frame_bar_thickness
        ...
        self.wall_pos = [0.3, 0, self.table_height + table_thickness/2+.075]

        # self.wall_pos = [0.3, 0, self.table_height + table_thickness/2+.075]
        # self.wall_top_pose = [0.3, 0, self.table_height + table_thickness/2+.525]
#
        # self.wall_pos = [0.3, 0, self.table_height + table_thickness/2 + self.wall_height/2]
        ...
        wall_rot = gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 0, 0), 0)  # Rotate 90 degrees about Y-axis
        wall_start_pose.r = wall_rot

        # wall_top_start_pose = gymapi.Transform()
        # wall_top_start_pose.p = gymapi.Vec3(*self.wall_top_pose)
        # wall_top_rot = gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 0, 0), 0) 
        # wall_top_start_pose.r = wall_top_rot
        ...
        num_wall_top_bodies = 0
        num_wall_top_shapes = 0

        # num_wall_top_bodies = self.gym.get_asset_rigid_body_count(wall_top_asset)
        # num_wall_top_shapes = self.gym.get_asset_rigid_shape_count(wall_top_asset)
        ...
        max_agg_bodies = num_franka_bodies  + num_wall_bodies + num_wall_top_bodies + num_table_bodies +   num_table_stand_bodies +  num_cubeA_bodies  + num_cylinder_bodies
        max_agg_shapes = num_franka_shapes  +   num_wall_shapes + num_wall_top_shapes + num_table_shapes +num_table_stand_shapes +num_cubeA_shapes +num_cylinder_shapes
        ...
        # print(f"Wall: Bodies: {num_wall_bodies}, Shapes: {num_wall_shapes}")
        # print(f"Wall: Bodies: {num_wall_bodies}, Shapes: {num_wall_shapes}")
        ...
        # # Print the total number of bodies and shapes for the aggregate
        # print(f"Total Aggregate Bodies: {max_agg_bodies}")
        # print(f"Total Aggregate Shapes: {max_agg_shapes}")
        # num_wall_bodies = self.gym.get_asset_rigid_body_count(wall_asset)
        # num_wall_shapes = self.gym.get_asset_rigid_shape_count(wall_asset)
        ...
        for i in range(self.num_envs):
            ...
            frame_start_pose.p = gymapi.Vec3(cubby_center[0], cubby_center[1], cubby_center[2] - self.frame_bar_length / 2)
            frame_start_pose.r = cubby_rotation1  # Apply the cubby rotation
            self._wall_id  =self.gym.create_actor(env_ptr, wall_asset, frame_start_pose,  "wall", i, 1, 0)
            # self._wall_top_id = self.gym.create_actor(env_ptr, wall_top_asset, wall_top_start_pose, "wall_top", i, 1, 0)
            # 3. Table
            ...
            # print("self.cubeA mass", props.mass)
    ...
    def init_data(self):
        ...
        print(f"Shape of global indices: {self._global_indices.shape}")
        print(f"Global indices for environment 0: {self._global_indices[0]}")
...
def compute_franka_reward(
    ...
    target_height = wall_pos[:, 2] + wall_center_to_square_top  + cubeA_size/2
