...
class CubeCubbyTask2(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            cube_goal = gymapi.Transform()
            cube_goal.p = gymapi.Vec3(0.0, 0.0, 0.0)
            cube_goal.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 0, 0), 1.5708)
            self.cube_goals.append([cube_goal.p.x, cube_goal.p.y, cube_goal.p.z,
                                    cube_goal.r.x, cube_goal.r.y, cube_goal.r.z, cube_goal.r.w])
            # 5. Cube B
    ...
    #     # TO DO probably have to do this for insertion.
    ...
    #     # print(f"cube_heights shape: {cube_heights.shape}")
    #     # print(f"env_ids shape: {env_ids.shape}")
    #     # print(f"sampled_cube_state shape: {sampled_cube_state.shape}")
...
def compute_franka_reward(
    ...
    align_reward = (1 - torch.tanh(10.0 * d_ab)) * cubeA_lifted # TODO multiply this by rotation flag
    ...
    #         print(f"    cubeA_quat: [{states['cubeA_quat'][i, 0].item()}, {states['cubeA_quat'][i, 1].item()}, {states['cubeA_quat'][i, 2].item()}, {states['cubeA_quat'][i, 3].item()}]")
    #         print(f"    quat_diff: [{quat_diff[i, 0].item()}, {quat_diff[i, 1].item()}, {quat_diff[i, 2].item()}, {quat_diff[i, 3].item()}]")
    #         print(f"    quat_align_reward[{quat_align_reward[i]}]")
    #         print(f"    rot_dist[{rot_dist[i]}]")
