...
class GeneralizedWallInsertHorizontal(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.reward_settings = {
            ...
            # "r_x_align_scale": self.cfg["env"]["r_x_align_scale"],
            # "r_yz_align_scale": self.cfg["env"]["r_yz_align_scale"],
    ...
    def _set_viewer_params(self):
        ...
        cam_pos = gymapi.Vec3(-2.0, -1.0, 2.5)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        triangle_start_pose.p = gymapi.Vec3(0.0, .2, self.table_height + table_thickness/2 + self.triangle_height/2)
    ...
    def _update_states(self):
        self.states.update({
            ...
            # "cube_to_hole_bottom_xyz": self._cube_state[:, :3] - self._hex_bottom_state[:, :3],
            ...
            "cube_hole_bottom_xyz": self._square_bottom_state[:, :3],
            # "cube_hole_bottom_xyz": self._hex_bottom_state[:, :3], # wall center
    ...
    def reset_idx(self, env_ids):
        ...
        # self._cube_hole_state[env_ids] = self._init_cube_hole_state[env_ids]
        # self._hex_hole_state[env_ids] = self._init_hex_hole_state[env_ids]
        # self._triangle_hole_state[env_ids] = self._init_triangle_hole_state[env_ids]
        ...
        # shapes are the last three indices
        multi_env_ids_objs_int32 = self._global_indices[env_ids, -3:].flatten()
    ...
    # def _reset_init_wall_state(self, obj, env_ids, check_valid=True):
    #     """
    ...
    #     init_state_dict = {'cube': self._init_cube_hole_state,
    #                         'hex': self._init_hex_hole_state,
    #                         'triangle': self._init_triangle_hole_state,
    #                         "wall_top": self._wall_top_state,
    #                         "wall_bottom": self._wall_bottom_state}
    ...
    #         random_x_offset = .25 * self.start_position_noise * (torch.rand_like(sampled_obj_state[active_idx, 0]) - 0.5)
    #         random_y_offset = .25 * self.start_position_noise * (torch.rand_like(sampled_obj_state[active_idx, 0]) - 0.5)
    ...
    #                 min_dist = (length_dict[obj.lower()] + length_dict[other_obj_name])/3
    ...
    def _reset_init_object_state(self, obj, env_ids, check_valid=True):
        ...
        for i in range(2000):
            ...
            random_y_offset = 2.5 * self.start_position_noise * (torch.rand_like(sampled_obj_state[active_idx, 0]) - 0.5)
...
def compute_franka_reward(
    ...
    # target z value for the object in the cubby at center point of object
  ...
  # How closely aligned obj is to cubby (only provided if obj is lifted) num_envs X 3 
    ...
    diff_obj_to_goal = obj_pos_xyz - obj_goal_position_xyz
    # then align to the final z position
    dist_obj_to_goal_z = torch.abs(diff_obj_to_goal[:, 2])
    obj_z_align_reward = (1 - torch.tanh(10.0 * dist_obj_to_goal_z)) * is_obj_lifted 
    obj_z_align_val_flag = diff_obj_to_goal[:, 2] < .05
    obj_z_align_positive_flag = 0 < diff_obj_to_goal[:, 2]
    obj_z_align_flag = obj_z_align_val_flag * obj_z_align_positive_flag * is_obj_lifted

    # then align along the y axis

    # start by getting to y halfway point
    dist_obj_to_goal_y_halfway = torch.abs(obj_pos_xyz[:, 1]-.5*obj_goal_position_xyz[:, 1])
    obj_y_align_halfway_reward = (1 - torch.tanh(10.0 * dist_obj_to_goal_y_halfway)) * is_obj_lifted * obj_z_align_flag
    obj_y_align_halfway_flag = dist_obj_to_goal_y_halfway < .01

    dist_obj_to_goal_y = torch.abs(diff_obj_to_goal[:, 1])
    obj_y_align_reward = (1 - torch.tanh(10.0 * dist_obj_to_goal_y)) * is_obj_lifted  * obj_z_align_flag * obj_y_align_halfway_flag
    obj_y_align_flag = dist_obj_to_goal_y < .05

    # then finally align along the x axis (insertion axis)
    dist_obj_to_goal_x = torch.abs(diff_obj_to_goal[:, 0])
    obj_x_align_reward = (1 - torch.tanh(10.0 * dist_obj_to_goal_x)) * is_obj_lifted * obj_z_align_flag * obj_y_align_flag
    obj_x_align_flag = dist_obj_to_goal_x < .075

    align_reward = obj_z_align_reward + obj_y_align_reward + obj_y_align_halfway_reward + obj_x_align_reward
    aligned_flag = obj_z_align_flag * obj_y_align_flag * obj_x_align_flag
    ...
    # dist_to_goal_yz = torch.norm(obj_pos_xyz[:, 1:] - obj_goal_position_xyz[:, 1:], dim=-1)
    # yz_align_reward = (1 - torch.tanh(10.0 * dist_to_goal_yz)) * is_obj_lifted
    # yz_aligned_flag = torch.abs(dist_to_goal_yz) < .03
    # dist_to_goal_x = torch.norm(obj_pos_xyz[:, 0] - obj_goal_position_xyz[:, 0], dim=-1)
    # x_align_reward = (1 - torch.tanh(10.0 * dist_to_goal_x)) * is_obj_lifted * yz_aligned_flag
    ...
    # obj XY alignment 
    # y_alignment  = torch.abs(obj_to_hole_bottom_xyz[:, 1]) < .05
    # x_alignment = torch.abs(obj_to_hole_bottom_xyz[:, 0]) < .075

    # xy_obj_to_obj_hole = obj_to_hole_bottom_xyz[:, :2]
    # dist_obj_to_obj_hole_xy = (torch.norm(xy_obj_to_obj_hole, dim=-1) < 0.02)

    # obj Z alignment 
    # dist_obj_to_obj_hole_z = torch.abs(obj_height_from_ground - target_obj_height) < 0.02
    
    # gripper releases obj 
    gripper_released_obj = (dist_obj_to_gripper > 0.08)
    ...
    obj_placement_complete =aligned_flag & gripper_released_obj
    ...
    # scaled_yz_align_reward = reward_settings["r_yz_align_scale"] * x_align_reward
    # scaled_x_align_reward = reward_settings["r_x_align_scale"] * x_align_reward

    scaled_alignment_reward = reward_settings["r_align_scale"] * align_reward
    # scaled_mid_reward = scaled_dist_reward + scaled_lifted_reward + scaled_yz_align_reward + scaled_x_align_reward#scaled_alignment_reward
    ...
    #         print(f"  dist_obj_to_obj_bottom:      {dist_obj_to_goal[i]}")
    #         # Print velocities
