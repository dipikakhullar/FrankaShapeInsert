...
"""
Cylinder and big wall the reward is only putting the cube in the cube hole
"""
...
class SplitShapeWallInsert(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        wall_asset_files_dict = {"hex_bottom": "urdf/separated_wall_shapes/hex_bottom.urdf",
                            "square_bottom": "urdf/separated_wall_shapes/square_bottom.urdf",
                            "wall_top": "urdf/separated_wall_shapes/wall_top.urdf",
                            "triangle_bottom": "urdf/separated_wall_shapes/triangle_top.urdf"
        ...
        self._wall_dims_dict["wall_top_height"] = 0.15
        self._wall_dims_dict["square_bottom_height"] = .45
        self._wall_dims_dict["square_bottom_z"] = .075
        self._wall_dims_dict["hex_bottom_z"] = .5
        self._wall_dims_dict["wall_top_z"] = .925
        self._wall_dims_dict["square_bottom_z_offset_from_center"] = -0.425
        self._wall_dims_dict["hex_bottom_z_offset_from_center"] = 0.0
        self._wall_dims_dict["wall_top_z_offset_from_center"] = 0.425
        self._wall_dims_dict["square_bottom_z_offset_from_hole"] = 0.075
        self._wall_dims_dict["hex_bottom_z_offset_from_hole"] = 0.05
        self._wall_dims_dict["wall_top_z_offset_from_hole"] = -0.075


        wall_pos = [0.3, 0, self.table_height + table_thickness/2 + self.wall_height/2]
        # wall_pos = [0.3, 0, self.reward_settings["table_height"] + table_thickness/2 + self._wall_dims_dict["wall_height"]/2]
        ...
        # 2. Custom Wall
        square_bottom_start_xyz = [wall_pos[0], wall_pos[1], wall_pos[2]+self._wall_dims_dict["square_bottom_z_offset_from_center"]]
        ...
        square_bottom_asset = self.gym.load_asset(self.sim, asset_root, wall_asset_files_dict['square_bottom'], wall_opts)

        # wall top
        wall_top_start_xyz = [wall_pos[0], wall_pos[1], wall_pos[2]+self._wall_dims_dict["wall_top_z_offset_from_center"]]
        ...
        wall_top_asset = self.gym.load_asset(self.sim, asset_root, wall_asset_files_dict['wall_top'], wall_opts)

        # hex bottom
        wall_opts.vhacd_params.resolution = 300000
        hex_bottom_start_xyz = [wall_pos[0], wall_pos[1], wall_pos[2]+self._wall_dims_dict["hex_bottom_z_offset_from_center"]]
        ...
        hex_bottom_asset = self.gym.load_asset(self.sim, asset_root, wall_asset_files_dict['hex_bottom'], wall_opts)
        ...
        max_agg_bodies = num_franka_bodies + square_bottom_bodies + hex_bottom_bodies + wall_top_bodies + num_table_bodies +   num_table_stand_bodies +  num_cubeA_bodies  + num_cylinder_bodies
        max_agg_shapes = num_franka_shapes  +  square_bottom_shapes + hex_bottom_shapes + wall_top_shapes + num_table_shapes +num_table_stand_shapes +num_cubeA_shapes +num_cylinder_shapes
        ...
        print(f"Cube A: Bodies: {num_cubeA_bodies}, Shapes: {num_cubeA_shapes}")
        print(f"Cylinder: Bodies: {num_cylinder_bodies}, Shapes: {num_cylinder_shapes}")
        ...
        for i in range(self.num_envs):
            ...
            # self._wall_id = self.gym.create_actor(env_ptr, wall_asset, wall_start_pose, "wall", i, 1, 0)
            ...
            # print("table_start_pose.p", table_start_pose.p)
            # print("table_stand_start_pose.p", table_stand_start_pose.p)
    ...
    def _update_states(self):
        self.states.update({
            ...
            "cube_A_to_square_bottom": self._cubeA_state[:, :3] - self._square_bottom_state[:, :3],
            ...
            "square_bottom_xyz": self._square_bottom_state[:, :3], # wall center
            "hex_bottom_xyz": self._square_bottom_state[:, :3],
            "square_bottom_z_offset_from_hole": torch.ones_like(self._eef_state[:, 2])*self._wall_dims_dict["square_bottom_z_offset_from_hole"],
    ...
    def _reset_init_object_state(self, obj, env_ids, check_valid=True):
        ...
        if obj.lower() == 'cube':
            ...
            y_offsets = torch.ones(num_resets, device=self.device)*.1
        elif obj.lower() == 'cylinder':
            this_obj_state_all = self._init_cylinder_state
            obj_heights = self.states["cylinder_length"]
            y_offsets = torch.ones(num_resets, device=self.device)*.1*-1
        else:
...
def compute_franka_reward(
    ...
    square_bottom_xyz = states["square_bottom_xyz"] # square bottom center
    hex_bottom_xyz = states["hex_bottom_xyz"]
    square_bottom_z_offset_from_hole = states["square_bottom_z_offset_from_hole"] 
    ...
    offset = torch.zeros_like(states["cube_A_to_square_bottom"])
    offset[:, 2] = square_bottom_z_offset_from_hole + cubeA_size/2
    if reward_settings["offset_positive"] > 0 :
    ...
    xy_cubeA_to_square_hole = states["cube_A_to_square_bottom"][:, :2]
    cubeA_align_square_hole = (torch.norm(xy_cubeA_to_square_hole, dim=-1) < 0.02)



    target_height = square_bottom_xyz[:, 2] + square_bottom_z_offset_from_hole + cubeA_size/2
    ...
    #     if cubeA_align_square_hole[i].item() or (cubeA_on_square_hole[i].item() and cubeA_lifted[i].item()):
    #         print(f"Env {i}:")
    ...
    #         print(f"  square_bottom_xyz:              [{states['square_bottom_xyz'][i, 0].item()}, {states['square_bottom_xyz'][i, 1].item()}, {states['square_bottom_xyz'][i, 2].item()}]")
    ...
    #         if stack_reward[i].item():
    #             # Update the success counter for environments where the stack_reward is true at the end of an episode
    #             episode_success_counter[i] += 1


    # if progress_buf[0] >= max_episode_length - 1 and global_episode_counter % 1024 == 0:
