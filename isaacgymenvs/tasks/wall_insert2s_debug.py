...
class WallInsert2S(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.cube_start_pose_xyz = [0, -.15, self.table_height + table_thickness/2 + self.cube_height/2]
        ...
        self.hex_start_pose_xyz = [0.0, 0.15, self.table_height + table_thickness/2 + self.hex_height/2]
        ...
        self.triangle_start_pose_xyz = [0.0, 0.0, self.table_height + table_thickness/2 + self.triangle_height/2]
...
def compute_franka_reward(
    ...
    if cube_task_complete_flag:
        ...
        hex_align_reward = (1 - torch.tanh(10.0 * dist_hex_to_goal)) * is_hex_lifted * cube_placement_complete
        ...
        hex_x_alignment = torch.abs(hex_to_hole_bottom_xyz[:, 1]) < .075
        ...
        scaled_dist_reward_hex = reward_settings["r_dist_scale"] * max_reward_bw_gripper_align
        scaled_lifted_reward_hex = reward_settings["r_lift_scale"] * is_hex_lifted
        scaled_alignment_reward_hex = reward_settings["r_align_scale"] * hex_align_reward
    ...
    for i in range(cube_placement_complete.shape[0]):
        cube_placement_progress = dist_cube_to_cube_hole_xy[i].item() or (dist_cube_to_cube_hole_z[i].item() and cube_lifted[i].item())
        ...
        if cube_placement_progress: # or hex_placement_progress:
            ...
            print("   x alignment cube", cube_x_alignment[i].item())
            print("   y alignment cube", cube_y_alignment[i].item())  
            ...
            # print(f"  dist_hex_to_hex_hole_z:    {dist_hex_to_hex_hole_z[i].item()} (cube_height_from_ground: {cube_height_from_ground[i].item()} vs target_cube_height: {target_cube_height[i].item()})")
            # print(f"  gripper_released_hex: {gripper_released_hex[i].item()}")
            # print("   x alignment hex", hex_x_alignment[i].item())
            # print("   y alignment hex", hex_y_alignment[i].item())  
